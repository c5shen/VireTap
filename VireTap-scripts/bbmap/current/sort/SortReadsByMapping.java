...
public class SortReadsByMapping {
	...
	public static void main(String[] args){
		...
		Data.sysout.println("Processed "+srt.processed+" reads; "+remaining+" remaining"+String.format(Locale.ROOT, " (%.2f%%)", remaining*rmult));
		if(MERGE_DUPLICATES){
			Data.sysout.println("Merged "+srt.merged2+" strict duplicates"+String.format(Locale.ROOT, " (%.2f%%)", srt.merged2*rmult));
			Data.sysout.println("Merged "+srt.merged+" duplicates"+String.format(Locale.ROOT, " (%.2f%%)", srt.merged*rmult));
			if(srt.paired && REMOVE_SINGLETON_DUPLICATES_OF_PAIRS){
				Data.sysout.println("Removed "+srt.removedSingletonDupe+" singleton duplicates of pairs"+
						String.format(Locale.ROOT, " (%.2f%%)", srt.removedSingletonDupe*rmult));
			}
		}
		if(FIX_SHORT_PAIRED_READS){
			Data.sysout.println("Removed "+srt.removedShort+" short reads"+String.format(Locale.ROOT, " (%.2f%%)", srt.removedShort*rmult));
			Data.sysout.println("Trimmed "+srt.basesOverlapping+" overlapping bases of "+srt.basesInitiallyMapped+" initially mapped"+
					String.format(Locale.ROOT, " (%.2f%%)", srt.basesOverlapping*bmult));
		}
		if(TRIM_LOW_QUALITY_TAILS){
			Data.sysout.println("Removed "+srt.removedLQ+" low-quality reads"+String.format(Locale.ROOT, " (%.2f%%)", srt.removedLQ*rmult));
			Data.sysout.println("Trimmed "+srt.basesRemoved+" low-quality bases of "+srt.basesMapped+" mapped"+
					String.format(Locale.ROOT, " (%.2f%%)", srt.basesRemoved*bmult));
		}
	}
	...
	public SortReadsByMapping(String fname1, String fname2, String outname_, int blocksize_){
		...
		blockwriter1=(fname1==null ? null : 
			new ReadStreamStringWriter(FileFormat.testOutput(fname1, FileFormat.SAM, null, true, true, false, true), null, true, 4, null, true));
		blockwriter2=(fname2==null ? null : 
			new ReadStreamStringWriter(FileFormat.testOutput(fname2, FileFormat.SAM, null, true, true, false, true), null, false, 4, null, true));
	}
	...
	public void process(){
		...
		ReadStreamWriter wt1=(fname1==null ? null : 
			new ReadStreamStringWriter(FileFormat.testOutput(fname1, FileFormat.SAM, null, true, true, false, true), null, true, 4, null, true));
		ReadStreamWriter wt2=(fname2==null ? null : 
			new ReadStreamStringWriter(FileFormat.testOutput(fname2, FileFormat.SAM, null, true, true, false, true), null, false, 4, null, true));
		...
		for(String key : keys){
			...
			if(b.added>MAX_BLOCKSIZE_TO_SORT){
				...
				if(TRIM_LOW_QUALITY_TAILS){
					int[] rvector=new int[4];
					int removedTemp=trimTails(list, TRIM_WINDOW, (byte)TRIM_QUALITY, rvector);
...
//	public long numSwapped=0;
	...
	public static float TRIM_QUALITY=7;
