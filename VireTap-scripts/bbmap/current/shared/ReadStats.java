...
//		}
	...
	public void writeGCToFile(String fname, boolean printZeros){
		...
		TextStreamWriter tsw=new TextStreamWriter(fname, overwrite, false, false);
		tsw.start();
		tsw.print("#Mean\t"+String.format(Locale.ROOT, "%.3f", GCMean)+"\n");
		tsw.print("#Median\t"+String.format(Locale.ROOT, "%.3f", GCMedian)+"\n");
		tsw.print("#Mode\t"+String.format(Locale.ROOT, "%.3f", GCMode)+"\n");
		tsw.print("#STDev\t"+String.format(Locale.ROOT, "%.3f", GCSTDev)+"\n");
		if(GC_PLOT_X){
...
//				tsw.print(i+"\t"+x+"\n");
				if(GC_PLOT_X){
					tsw.print(String.format(Locale.ROOT, "%.1f\t%d\t", i*gcMult, x));
					
					tsw.print(String.format(Locale.ROOT, "%.3f\t", sum*fractionMult));
					...
					tsw.print(String.format(Locale.ROOT, "%.1f\t%d\n", i*gcMult, x));
				}
			}
		}
		tsw.poison();
